...
bool MAX7219_Digit(uint8_t index, uint8_t digit, int8_t value)
{
	...
	if (!MAX7219_Write(index, digit + 1, value))
		return false;
...
bool MAX7219_Decode(uint8_t index, uint8_t value)
{
	...
	if (!MAX7219_Write(index, MAX7219_REG_DECODE, value))
		return false;
...
bool MAX7219_Intensity(uint8_t index, uint8_t value)
{
	...
	if (!MAX7219_Write(index, MAX7219_REG_INTENSITY, value))
		return false;
...
bool MAX7219_ScanLimit(uint8_t index, uint8_t value)
{
	...
	if (!MAX7219_Write(index, MAX7219_REG_SCANLIMIT, value))
		return false;
...
bool MAX7219_ShutDown(uint8_t index, uint8_t value)
{
	...
	if (!MAX7219_Write(index, MAX7219_REG_SHUTDOWN, value))
		return false;
...
bool MAX7219_Test(uint8_t index, uint8_t value)
{
	...
	if (!MAX7219_Write(index, MAX7219_REG_TEST, value))
		return false;
...
static bool SPI_Tx(uint8_t data)
{
	if (HAL_SPI_Transmit(Max7219_SPI, &data, 1, HAL_MAX_DELAY) != HAL_OK)
		return false;
	return true;
}
