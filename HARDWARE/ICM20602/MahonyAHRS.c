...
#define twoKpDef (2.0f * 8.584f)	 // 2 * proportional gain
#define twoKiDef (2.0f * 18.421264f) // 2 * integral gain
...
volatile float roll, pitch, yaw;										   // Euler Angle
volatile float integralFBx = 0.0f, integralFBy = 0.0f, integralFBz = 0.0f; // integral error terms scaled by Ki 角度误差积分项
...
void MahonyAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay, float az)
{
	...
	// Integrate rate of change of quaternion
	gx *= (0.5f * (1 / sampleFreq)); // pre-multiply common factors
	gy *= (0.5f * (1 / sampleFreq));
	gz *= (0.5f * (1 / sampleFreq));
	qa = q0;
	qb = q1;
	qc = q2;
	q0 += (-qb * gx - qc * gy - q3 * gz);
