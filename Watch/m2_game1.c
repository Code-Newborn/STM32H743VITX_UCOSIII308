...
display_t game1_draw() {
    ...
    {
        LOOP(BLOCK_ROWS, y) {  // 小球与砖块的位置关系如果满足
    ...
    LOOP(BLOCK_COLS, x) {
        LOOP(BLOCK_ROWS, y) {
            if (!blocks[idx]) draw_bitmap(x * 4, (y * 4) + 8, block, 3, 8, NOINVERT, 0);
            idx++;
        }
    }
    ...
    if (score >= BLOCK_COUNT) draw_string_P(PSTR(STR_WIN), false, 50, 32);  // 绘制游戏胜利

    // No lives left (255 because overflow)
    if (lives == 255) draw_string_P(PSTR(STR_GAMEOVER), false, 34, 32);  // 绘制游戏结束
