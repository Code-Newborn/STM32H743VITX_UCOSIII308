...
bool menu_isOpen() { return menuData.isOpen; }
...
bool exitSelected() { return menuData.selected == menuData.optionCount - 1; }
/*
...
*/
static void clear() { memset(&menuData.func, 0, sizeof(menuFuncs_t)); }

void addBackOption() { setMenuOption_P(menuData.optionCount - 1, menuBack, menu_exit, back); }
...
void beginAnimation(menu_f onComplete) { animation_start(onComplete, ANIM_MOVE_OFF); }
// 带打开动画的函数，动画执行完后执行onComplete函数
void beginAnimation2(menu_f onComplete) { animation_start(onComplete, ANIM_MOVE_ON); }
...
void doAction(bool anim) { loader(OPERATION_ACTION, menuData.selected, anim); }
