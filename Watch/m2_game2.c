...
static display_t draw() {
    ...
    if (myCar.hit && now - hitTime >= 1000) myCar.hit = false;
    ...
    // Draw my car
    if (!myCar.hit || (myCar.hit && (now & 64))) draw_bitmap(0, myCar.y + quakeY, carImg, 15, 16, NOINVERT, 0);
    ...
    if (lives != 255) {
        ...
        LOOP(3, i) {
            ...
            if (dotX[i] >= FRAME_WIDTH && dotX[i] < 255 - 8) dotX[i] = FRAME_WIDTH;
