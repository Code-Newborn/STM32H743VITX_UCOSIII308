...
static void BSP_OS_DynamicTickInit(void)
{
    ...
    if (rcc_clk_cfg.APB1CLKDivider == RCC_HCLK_DIV1)
    { /* Configure the timer prescaler. See Note (1).         */
...
OS_TICK OS_DynTickGet(void)
{
    ...
    { /* Counter value already overflowed.                    */
        return (TickDelta);
    }
...
OS_TICK OS_DynTickSet(OS_TICK ticks)
{
    ...
    if ((tmrcnt >= TIMER_COUNT_MAX) || /* If the delay exceeds our timer's capacity,       ... */
        (tmrcnt == 0u))
    {                             /* ... or the kernel wants an indefinite delay.         */
